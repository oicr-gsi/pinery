...
public class LaneProvenanceResourceTest {
	...
	public void testEmptyResultSet() throws Exception {
		...
		mockMvc.perform(get("/provenance/latest/lane-provenance").accept(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isOk())
	...
	public void testJsonArraySize() throws Exception {
		...
		mockMvc.perform(get("/provenance/latest/lane-provenance").accept(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(jsonPath("$", hasSize(100)));
	}

  /**
   * Test that the v1 lane provenance object (and version hash) has not changed. This method should be replicated for each new
   * provenance version added
   */
  @Test
  public void testV1ProvenanceTransform() {
    VersionTransformer<LaneProvenance> transformer = LaneProvenanceResource.transformers.get("v1");
