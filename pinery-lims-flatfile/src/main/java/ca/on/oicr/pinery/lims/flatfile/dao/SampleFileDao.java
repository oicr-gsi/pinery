...
public class SampleFileDao implements SampleDao {
  ...
  private static final String queryFilteredSamples =
      queryAllSamples + " WHERE archived LIKE ?" + " AND createdDate < ?" + " AND createdDate > ?";

  private static final String querySampleById = queryAllSamples + " WHERE id LIKE ?";

  private static final String queryTypeList =
  ...
  private static final RowMapper<Type> typeMapper =
      new RowMapper<Type>() {
  ...
  private static final RowMapper<Sample> sampleMapper =
      new RowMapper<Sample>() {
        ...
        public Sample mapRow(ResultSet rs, int rowNum) throws SQLException {
          ...
          s.setVolume(ModelUtils.parseFloatOrNull(rs.getString("volume")));
          s.setConcentration(ModelUtils.parseFloatOrNull(rs.getString("concentration")));
  ...
  private Object[] makeQueryParams(
      Boolean archived,
      Set<String> projects,
      Set<String> types,
      ZonedDateTime before,
      ZonedDateTime after) {
    int paramCount =
        3 + (projects == null ? 0 : projects.size()) + (types == null ? 0 : types.size());
    Object[] queryParams = new Object[paramCount];
    queryParams[0] = archived == null ? "%" : archived.toString();
    queryParams[1] =
        before == null ? ZonedDateTime.now().plusDays(1).toString() : before.toString();
    queryParams[2] =
        after == null ? ZonedDateTime.now().withYear(2000).toString() : after.toString();
    int paramPos = 3;
    if (projects != null) {
