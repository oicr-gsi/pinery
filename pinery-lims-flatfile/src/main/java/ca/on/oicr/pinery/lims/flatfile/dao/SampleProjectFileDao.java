...
public class SampleProjectFileDao implements SampleProjectDao {
  ...
  private static final String querySampleStats = "SELECT projectName, COUNT(*) AS count,"
  ...
  public List<SampleProject> getAllSampleProjects() {
    ...
    Map<String, SampleProject> projectsByName = projects.stream().collect(Collectors.toMap(SampleProject::getName, Function.identity()));
    template.query(querySampleStats, rs -> {
